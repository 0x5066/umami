...
extern PlayerCore* PlayerCore;  // declare external reference
...
bool renderVis(SDL_Renderer* renderer, Skin& skin, const UIElement& elem, int parentX, int parentY, int parentW, int parentH) {
    ...
    PlayerCore->GetSaData(sample);
        ...
        #endif // __LINUX
        int i = (i = vx & 0xfffffffc); // Limiting i to prevent out of bounds access
            if (sa_thick == true) {
